...
class GraphWaveNet(nn.Module):
    def __init__(self, 
                 num_nodes, in_channels, out_channels, out_timesteps, 
                 dilations=[1, 2, 1, 2, 1, 2, 1, 2], adptive_embeddings=10, dropout=0.3, 
        ...
        self.end1 = nn.Conv2d(in_channels=skip_channels, out_channels=end_channels, kernel_size=(1, 1))
        self.end2 = nn.Conv2d(in_channels=end_channels, out_channels=out_channels * out_timesteps, kernel_size=(1, 1))
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        edge_index_adp = torch.tensor(edge_index_adp)
        edge_weight_adp = torch.tensor(edge_weight_adp)
        ...
        for k in range(self.num_dilations):
            ...
            data = self.batch_timesteps(g, edge_index, edge_weight)
        ...
        x = torch.relu(x)
        x = self.end2(x)
        x = x.reshape(*batch_size, self.out_timesteps, self.out_channels, self.num_nodes).transpose(-1, -2)
