...
class gwnet(nn.Module):
    ...
    def forward(self, input):
        ...
        for i in range(self.blocks * self.layers):
            ...
            residual = x
            print("i, shape", i, residual.shape)
            # dilated convolution
            ...
            print("After iter {}, shape is {}".format(i, x.shape))

        x = F.relu(skip)
        x = F.relu(self.end_conv_1(x))
        print("After end conv 1, shape is {}".format(x.shape))
        x = self.end_conv_2(x)
        print("After end conv 2, shape is {}".format(x.shape))
